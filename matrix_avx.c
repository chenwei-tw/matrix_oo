...
static bool avx_prefetch_mul(Matrix **dst, const Matrix *l, const Matrix *r)
{
    if(l->col != r->row) return false;
    int src1_h = (l->row % 8 == 0) ? l->row : l->row + (8 - l->row % 8);
    int src2_w = (r->col % 8 == 0) ? r->col : r->col + (8 - r->col % 8);
    ...
    for (int i = 0; i < src1_h; i += 8) {
        for (int j = 0; j < src2_w; j += 8) {
            ...
            for (int k = 0; k < src2_h; k += 8) {
#define AVX_PFDIST 8
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 0][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 1][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 2][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 3][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 4][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 5][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 6][j]), _MM_HINT_T1);
                _mm_prefetch(&(PRIV(r)->values[k + AVX_PFDIST + 7][j]), _MM_HINT_T1);
...
MatrixAlgo prefetchAVXMatrixProvider = {
    .create = create,
    .assign = assign,
    .equal = equal,
    .mul = avx_prefetch_mul,
    .display = display,
};
